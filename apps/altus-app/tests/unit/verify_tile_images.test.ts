...
let getTitles: () => Promise<Title[]>;

before(async () => {
  const userMod = await import('../../src/stores/user_store.ts');
  UserStore = userMod.default;
  await UserStore.init();
  assert.ok(UserStore.isLoggedIn(), 'must be logged in');

  const productMod = await import('../../src/stores/product_store.ts');
  ProductStore = productMod.default;
  await ProductStore.init();

  const xcloudMod = await import('../../src/lib/xcloud_api.ts');
  getTitles = xcloudMod.getTitles;
});

void describe('verify_tile_images', () => {
  void it('dumps full info for all titles missing a tile image', async () => {
    ...
    console.log(`\n=== Missing tile image: ${missing.length} ===\n`);
    for (const m of missing) {
      console.log(JSON.stringify({ ...m.title, ...m.product }, null, 2));
      console.log('---');
    }
  });
});
