...
interface ConsoleStoreModule {
  init: () => void;
  fetch: () => Promise<void>;
}

interface GameStoreModule {
  init: () => void;
  fetch: () => Promise<void>;
}

let UserStore: UserStoreModule;
let ConsoleStore: ConsoleStoreModule;
let GameStore: GameStoreModule;

before(async () => {
  const userMod = await import('../../src/stores/user_store.ts');
  UserStore = userMod.default;
  const consoleMod = await import('../../src/stores/console_store.ts');
  ConsoleStore = consoleMod.default;
  const gameMod = await import('../../src/stores/game_store.ts');
  GameStore = gameMod.default;
});

void describe('UserStore.init (live)', () => {
  void it('loads credentials and acquires a streaming token', async () => {
    await UserStore.init();

    assert.ok(UserStore.isLoggedIn(), 'should be logged in after init');
    assert.ok(
      UserStore.getAccessToken().length > 0,
      'access token should be populated'
    );
  });
});

void describe('ConsoleStore.fetch (live)', () => {
  void it('fetches consoles using xHome credential', async () => {
    ConsoleStore.init();
    await ConsoleStore.fetch();
    // If fetch completes without throwing, xHome credential worked
    assert.ok(true, 'ConsoleStore.fetch completed successfully');
  });
});

void describe('GameStore.fetch (live)', () => {
  void it('fetches games using xgpuweb credential', async () => {
    GameStore.init();
    await GameStore.fetch();
    // fetch catches errors internally — if no xgpuweb token, it logs and returns
    assert.ok(true, 'GameStore.fetch completed without unhandled error');
  });

  void it('logs a sample title and console', async () => {
    const { getRecentTitles, getConsoles } =
    ...
    if (consoles.length > 0) {
      ...
      console.log(JSON.stringify(consoles[0], null, 2));
    }
  });
});
